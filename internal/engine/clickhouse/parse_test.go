...
func TestParseArrayJoin(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// Check for RangeFunction (our representation of ARRAY JOIN)
			hasArrayJoin := false
			for _, item := range selectStmt.FromClause.Items {
				if rangeFunc, ok := item.(*ast.RangeFunction); ok {
					hasArrayJoin = true
					
					// Verify the function name is "arrayjoin"
					if rangeFunc.Functions != nil && len(rangeFunc.Functions.Items) > 0 {
						if funcCall, ok := rangeFunc.Functions.Items[0].(*ast.FuncCall); ok {
							if funcCall.Func.Name != "arrayjoin" {
								t.Errorf("Expected function name 'arrayjoin', got '%s'", funcCall.Func.Name)
							}
						}
					}
					
					// Verify lateral flag for LEFT ARRAY JOIN
					if test.expectJoinType == "LEFT" && !rangeFunc.Lateral {
						t.Error("Expected Lateral=true for LEFT ARRAY JOIN")
					}
				}
			}
